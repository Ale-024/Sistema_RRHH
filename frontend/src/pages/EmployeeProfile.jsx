  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6 animate-in fade-in duration-500">
      <div>
        ...
        <p className="text-slate-500 dark:text-slate-400 mt-1">Consulta y actualiza tu información personal</p>
      </div>

      {message.text && (
        <div className={`p-4 rounded-lg ${message.type === 'success' ? 'bg-green-50 dark:bg-emerald-500/10 text-green-700 dark:text-emerald-400' : 'bg-red-50 dark:bg-red-500/10 text-red-700 dark:text-red-400'}`}>
          {message.text}
        </div>
      )}
      ...
      <div className="bg-white dark:bg-slate-800 shadow-sm rounded-xl border border-slate-200 dark:border-slate-700 overflow-hidden">
        {/* Header Info */}
        <div className="p-6 sm:p-8 bg-slate-50 dark:bg-slate-800/60 border-b border-slate-200 dark:border-slate-700 flex items-center space-x-6">
          <div className="w-24 h-24 rounded-full bg-slate-200 dark:bg-slate-700 flex items-center justify-center border-4 border-white shadow-sm">
            <User className="w-10 h-10 text-slate-400" />
          </div>
          <div>
            <h2 className="text-xl font-bold text-slate-900 dark:text-slate-100">{profile?.nombres} {profile?.apellidos}</h2>
            <p className="text-brand-blue font-medium">{profile?.puesto?.titulo}</p>
            <p className="text-sm text-slate-500 dark:text-slate-400 mt-1">{profile?.puesto?.departamento?.nombre}</p>
          </div>
        </div>
        ...
        <form onSubmit={handleSave} className="p-6 sm:p-8 space-y-6">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
              <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">DNI</label>
              <input type="text" disabled value={profile?.dni || ''} className="w-full p-2.5 bg-slate-100 dark:bg-slate-700 border border-slate-300 dark:border-slate-600 rounded-lg text-slate-500 dark:text-slate-400 cursor-not-allowed" />
            </div>
            
            <div>
              <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">Fecha de Ingreso</label>
              <input type="text" disabled value={profile?.fecha_ingreso ? new Date(profile.fecha_ingreso).toLocaleDateString() : ''} className="w-full p-2.5 bg-slate-100 dark:bg-slate-700 border border-slate-300 dark:border-slate-600 rounded-lg text-slate-500 dark:text-slate-400 cursor-not-allowed" />
            </div>

            <div className="md:col-span-2 pt-4 border-t border-slate-100 dark:border-slate-700/60">
              <h3 className="text-lg font-medium text-slate-900 dark:text-slate-100 mb-4">Información de Contacto (Editable)</h3>
            </div>
            ...
            <div>
              ...
              <input 
                ...
                className="w-full p-2.5 bg-white dark:bg-slate-800 border border-slate-300 dark:border-slate-600 rounded-lg focus:ring-2 focus:ring-brand-blue focus:border-brand-blue transition-all" 
              />
            </div>
      ...
      <div className="bg-white dark:bg-slate-800 shadow-sm rounded-xl border border-slate-200 dark:border-slate-700 overflow-hidden">
        <div className="p-6 sm:p-8 bg-slate-50 dark:bg-slate-800/60 border-b border-slate-200 dark:border-slate-700 flex items-center">
          <Lock className="w-6 h-6 text-slate-400 mr-3" />
          <h2 className="text-xl font-bold text-slate-900 dark:text-slate-100">Cambiar Contraseña</h2>
        </div>
        
        <form onSubmit={handlePasswordChange} className="p-6 sm:p-8 space-y-6">
          {passwordMessage.text && (
            <div className={`p-4 rounded-lg ${passwordMessage.type === 'success' ? 'bg-green-50 dark:bg-emerald-500/10 text-green-700 dark:text-emerald-400' : 'bg-red-50 dark:bg-red-500/10 text-red-700 dark:text-red-400'}`}>
              {passwordMessage.text}
            </div>
          )}
